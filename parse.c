    ...
    if (this_token->ttype == TOK_ID) {
        ...
        if (this_token->ttype == TOK_LPAREN) {
            ...
            if (this_token->ttype == TOK_UMINUS || this_token->ttype == TOK_NOT) {
                ...
                advance_lexer();
                // idk if i should calloc
                node_t *child = build_exp();
                switch(unary) {
                    case TOK_UMINUS :
                        unop->tok = unary;
                        unop->node_type = NT_INTERNAL;
                        unop->val.ival = -1 * child->val.ival;
                        unop->type = INT_TYPE;
                ...
                if (this_token->ttype == TOK_RPAREN) {
                    ...
                    switch(temptype) {
                        case TOK_PLUS :
                        binaryop->type = INT_TYPE;
                        binaryop->children[0] = left;
                        binaryop->children[1] = right;
                        binaryop->val.ival = left->val.ival + right->val.ival;
                        break;
                    }
                    advance_lexer();
                    return binaryop;
...
/*
static node_t *build_exp_helper() {
